...
class AppIntroSlider extends React.Component {
    ...
    render() {
        // Separate props used by the components to props passed to FlatList
        ...
        // Merge components width and user-defined extraData
        const extra = merge_extradata_1.default(extraData, this.state.width);
        return (<react_native_1.View style={styles.flexOne}>
        <react_native_1.FlatList ref={(ref) => (this.flatList = ref)} data={this.props.data} horizontal pagingEnabled showsHorizontalScrollIndicator={false} bounces={false} style={styles.flatList} renderItem={this._renderItem} onScroll={this._onMomentumScrollEnd} extraData={extra} onLayout={this._onLayout}
        // make sure all slides are rendered so we can use dots to navigate to them
        initialNumToRender={data.length} {...otherProps}/>
        {(this.state.activeIndex !== 2) ? this._renderPagination() : null}
      </react_native_1.View>);
    }
}
